...
void load_vars(Files& files, std::unordered_map<std::string, Var>& var_map, std::string terminator) {
  ...
  while (std::getline(files.src, line)) {
    ...
    // otherwise if the line is not empty then parse it
      ...
      std::string scat, stype, sname, srows, scols;
      Category cat;
      iss >> scat >> stype >> sname;
      if (scat == "mat") {
        iss >> srows >> scols;
        cat = MAT;
      } else if (stype == "arr") {
      ...
      var_map[sname] = (Var){ cat, stype, srows, scols };
    }
  }
}
...
void write_exo_input(Files& files, State& state) {
  ...
  for (auto& var : state.in_vars) {
    if (var.second.cat == VAR) {
      files.exo << "  " << var.second.type<< " " << var.first << ";" << std::endl;
    } else if (var.second.cat == ARR) {
      files.exo << "  " << var.second.type << " " << var.first << "[" << var.second.rows << "];" << std::endl;
    } else if (var.second.cat == MAT) {
      files.exo << "  " << var.second.type << " " << var.first << "[" << var.second.rows << " * " << var.second.cols << "];" << std::endl;
    }
  }
  files.exo << "};" << std::endl;
  ...
  files.exo << "\tfp = fopen(\"prover_verifier_shared/" << state.name << "_tb.inputs\", \"r\");" << std::endl;
...
void compile_output(Files& files, State& state) {
  ...
  write_pequin_input_output(files.tsc, state, false);
