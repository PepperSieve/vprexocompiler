...
void compute(struct In *input, struct Out *output) {
	...
	int ss = 2;
	int i = 2;
	int next_x_ti = slot( input->X, i);
	int next_y_ti = slot( input->Y, i);
	int prod_ti = X_PROD(x1, y1, x2, y2, next_x_ti, next_y_ti);
	slot( stack_x, ss) = next_x_ti;
	slot( stack_y, ss) = next_y_ti;
	ss = ss + 1;
	x1 = x2;
	y1 = y2;
	x2 = next_x_ti;
	...
	[[buffet::fsm(2*MAX_N)]]
	while(i < n){
		next_x_ti = slot( input->X, i);
		next_y_ti = slot( input->Y, i);
