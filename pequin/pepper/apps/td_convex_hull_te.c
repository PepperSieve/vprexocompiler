...
void compute(struct In *input, struct Out *output) {
	int ITER1; int ITER2;
	int *public_info[3] = {input->X, input->Y, input->n};
	ghost_t ghost[1];
	int len[3] = {MAX_N, MAX_N, 1};
	...
	int stack_y[MAX_N];
	for (ITER1 = 0; ITER1 < MAX_N; ITER1++) {
		stack_y[ITER1] = ghost[0].values[0 + MAX_N + MAX_N + ITER1];
	}
	int stack_x[MAX_N];
	for (ITER1 = 0; ITER1 < MAX_N; ITER1++) {
		stack_x[ITER1] = ghost[0].values[0 + MAX_N + MAX_N + MAX_N + ITER1];
	}
	int n = input->n[0];
